  ...
  effects: {
    ...
    async submitReturnRequest(payload: { orderNumber: string; lineItems: Array<{ id: number; category: string; reason: string }>; email: string; cookies?: string }, rootState: any) {
      try {
        ...
        const orderReturnResponse = await fameApi.submitReturnRequest(payload.orderNumber, lineItemTransformed as OrderReturnLineItemRequest[], payload.email, payload.cookies);
