...
import {
    componentShouldBeInUrl,
    generateProductDetailUrl,
    getComponentsFromSlug,
    getIdFromSlug,
...
class Product extends React.Component<Props, State> {
    public static getDerivedStateFromProps(nextProps: Props, prevState: State): Partial<State> {
        ...
        if (product) {
            ...
            const colors = colorIdOrCodes.map((codeOrId) =>
                product.components.find(
                    (x) =>
                        x.code === codeOrId ||
                        x.cartId === parseInt(codeOrId, 10) ||
                        x.meta.colorCode === codeOrId ||
                        x.meta.colorId === parseInt(codeOrId, 10)
                )
            );
            ...
            if (currentCustomizedProduct) {
                ...
                selectedComponents = [
                    ...currentCustomizedProduct.components.filter((x) => !componentShouldBeInUrl(x)),
                    ...selectedComponents,
                ];
            }
    ...
    private goToCustomizationStep = (group: Group, sectionGroup: SectionGroup | null, customizedProduct: CustomizedProduct, trackingLabel: string) => {
        ...
        if (group.sectionGroups.length === 1) {
            ...
            this.props.history.push(
                generateProductDetailUrl(customizedProduct, '/customize/' + group.slug + '/' + firstOrSelectedSectionGroup.slug)
            );
        }
    }
                        ...
                        render={({ match: submatch }) => {
                            ...
                            return (
                                <GalleryModal
                                    ...
                                    productListSummaries={products.productListSummmaries}
