...
class SearchResultPage extends React.PureComponent<Props> {
    ...
    public getTitle() {
        ...
        if (type === SearchPageType.SEARCH && query) {
            return (
                <h1><FormattedMessage id={'SearchPage.Title'} defaultMessage={'Search Results for "{query}"'} values={{query}} /></h1>
            );
        } else if (type === SearchPageType.CUSTOM_CLOTHING_SEARCH) {
            return null;
        } else if (facetDetails) {
            return (
                <h1>{facetDetails.title}</h1>
            );
        } else {
            return (
                <h1><FormattedMessage id={'SearchPage.Title'} defaultMessage={'Search Results'} /></h1>
            );
        }
    }
    ...
    public renderSearchFilterMobile() {
        ...
        if (searchResult && searchResult.facetGroups) {
            ...
            selectedFacetColorCircles = facets
                .filter((x) => x.facetMeta && x.facetMeta.hex)
                .map((x) => <FabricOrColorCircle key={x.facetMeta!.hex} component={x} style={{width: 16, height: 16, marginTop: 4, marginRight: 4}} />);
        }
            ...
            <div  className="MobileFilterOverview" onClick={onShowMobileFilters}>
                ...
                `}</style>
                    ...
                    <div className="MobileFilterOverview__Line MobileFilterOverview__Line--title">
                        <span className="title"><FormattedMessage id={'SearchPage.Filter'} defaultMessage={'Filter by:'} /></span>
                        <BackArrow style={{ width: 12, height: 12, transform: 'rotate(180deg)', marginLeft: 16, marginTop: 6}} />
                    </div>
                    { selectedFacetNames && <div className="MobileFilterOverview__Line">
                        <span className="selection">{selectedFacetNames}</span>
                    </div>
                    }
                    { selectedFacetColorCircles.length > 0 && <div className="MobileFilterOverview__Line">
    ...
    public render() {
        ...
        const results = searchResult.results.map((x) => (
            <UserContext.Consumer key={x.pid}>
                {(user) => (
                    <ProductCard key={x.pid} product={x} siteVersion={siteVersion} user={user} overlayText={edgeToEdge} />
                )}
        ...
        return (
            <BaseLayout>
                ...
                <div className={edgeToEdge ?  '' : 'grid'}>
                    <Desktop>
                        {!edgeToEdge && this.renderSidebar()}
                    </Desktop>

                    <section className={'Container'} ref={this.resultsContainerRef}>
                        {isLoading && <div className={classnames('page-loader', {'page-loader--overlay': results.length > 0})}>
                        ...
                        {results.length > 0 && (                                
                            <InfiniteScroll
                                ...
                                initialLoad={false}
                                threshold={300}
                                loader={<div key={'loader'} className={classnames('loading-more-loader', {'loading-more-loader--background': edgeToEdge})}><Spinner width={32} height={32} color={'grey79'} /></div>}
