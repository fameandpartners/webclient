  ...
  private renderSearchFilters = () => {
    ...
    if (facetConfiguration) {
      return facetConfiguration.map((facetCategory: FacetCategory, i) => {
        const groupsInCategory = facetCategory.facetGroupIds.filter((fgId) => !onlyShowFacetGroups || onlyShowFacetGroups.includes(fgId)).map((fgId) => facetGroups[fgId]);
        ...
        return <FilterGroup key={facetCategory.name} title={facetCategory.hideHeader ? undefined : facetCategory.name} onSelected={onSelected} options={options} resetFilters={onResetFilters ? () => onResetFilters(facetCategory) : undefined} />;
      });
    }
