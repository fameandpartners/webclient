...
import {
    convertCmToTotalInch,
    convertTotalInchesToFeetInchString,
    convertTotalInchToCm,
    extractCmAndTotalInches,
} from '@common/utils/size-helper';
...
class SizeSelectionSection extends React.PureComponent<Props, State> {
    ...
    public render() {
        ...
        const sizeOptions = customizedProduct.product.components
            .filter((c) => this.props.section.options.map(mapToCode).includes(c.code))
            .sort(sortByOrder);
        ...
        const inches = [
            {
                ...
                value: 'INCH',
                options: Array
                            .from({ length: inchesCount + 1 }, (k, i) => i + customizedProduct.product.size.minHeightInch)
                            .map((x) => {
                                return {
            ...
            <React.Fragment>
                ...
                <div className={classnames('size-selection', { 'size-selection--error': this.props.showErrors })}>
                        ...
                        <InputWrapper
                            ...
                            inlineError={
                                customizedProduct.heightUnit === HeightUnitType.CM
                                    ...
                                    />
                                    : <FormattedMessage
                                            id="PDP.SizeSelection.Height.ErrorInch"
                                            defaultMessage={'Please select your height'}
                                            values={{ min: customizedProduct.product.size.minHeightCm, max: customizedProduct.product.size.maxHeightCm }}
                                    />
                            }
                        >
                            <div className="height-section">
                                ...
                                >
                                    {customizedProduct.heightUnit === HeightUnitType.INCH ? (
                                        <Dropdown
                                            placeholder="Height"
                                            optionGroups={inches}
                                            selected={this.state.inches ? this.state.inches.toString() : null}
                                            onSelect={this.updateInches}
                                            isError={(this.state.showSizeError || this.props.showErrors) && !validSize}
                                        />
                                    ) : (
                                        <Input
                                            type="text"
                                ...
                                <Toggle
                                    initialValue={customizedProduct.heightUnit === HeightUnitType.CM}
                                    leftTitle={'IN'}
                                    leftValue={HeightUnitType.INCH}
                                    rightTitle={'CM'}
                                    rightValue={HeightUnitType.CM}
                                    onToggle={this.onSelectHeightUnit}
                                />
                            </div>
                        </InputWrapper>
                    ...
                    <div className="size">
                        ...
                        <InputWrapper
                            className="size-section-wrapper"
                            error={(this.state.showSizeError || this.props.showErrors) && !validSize}
                            inlineError={
                                <FormattedMessage
                                    id="PDP.SizeSelection.Size.Error"
