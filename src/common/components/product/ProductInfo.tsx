...
import {
    ...
    filterByCompatibleWithSelection,
    totalPrice,
    isNewProduct,
...
class ProductInfo extends React.PureComponent<Props, State> {
    ...
    public render() {
        ...
        const makingComponents = product.components
            .filter(filterByComponentType(ComponentType.Making)).sort(sortByOrder)
        ...
        return (
            <div className={'ProductInfo'}>
                ...
                {isAvailable && (
                    <Desktop>
                        <div className="DressTitleWrapper">
                            <h1 className="DressTitleWrapper__Title">
                                {dressTitle}
                            </h1>
                ...
                {isAvailable && (
                    <div className="customization-overview">
                        <CustomizationOverview
                            ...
                            canCustomize={isAvailable}
                            startCustomize={(group) => goToCustomizationStep(group, null, currentCustomizedProduct, 'Customization Overview')}
                        />
                    </div>
                )}
                ...
                {isAvailable && (
                    <Button
                        ...
                        errorText={
                            <FormattedMessage
                                id="Pdp.AddToCart.Error"
                                defaultMessage="Oh no, there was a problem adding this to your bag. Why don't you try that again?"
                            />
                        }
                    >
                        <Desktop>
                        ...
                        <Mobile>
                            <FormattedMessage
                                id="Pdp.AddToCart.WithPrice"
                                defaultMessage="{price} - ADD TO BAG"
                                values={{ price: `$${(total / 100).toFixed(2)}` }}
                            />
                        </Mobile>
                    </Button>
                )}

                {product.paymentMethods.afterPay && <p className="auxilary-info"><AfterpayTeaser total={total} /></p>}

                {(returnText) && (
                    <p className="auxilary-info">
                        {returnText && <span dangerouslySetInnerHTML={{ __html: returnText }} />}
                    </p>
                )}
            ...
            {isAvailable && (
                    <div className="share-moodboard">
                       ...
                       {showBuySwatch && <Link className="no-underline icon-text" to="/custom-clothes/order-swatches" target="_blank">
                            <SwatchesIcon
                                alt=""
...
<SiteVersionContext.Consumer>
                    {(siteVersion) => (
                        <ShareModal
                            ...
                            url={`${getBaseUrl(siteVersion).frontend}${generateProductDetailUrl(currentCustomizedProduct, '')}`}
