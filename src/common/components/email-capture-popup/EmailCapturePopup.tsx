    ...
    return (
      <React.Fragment>
        ...
        <div className="EmailCaptureModal__Form">
          {this.props.emailCapture && React.cloneElement(this.props.emailCapture, { ...this.props.emailCapture.props, siteVersion: this.props.siteVersion, onSignupSuccess: () => this.onSubmit() })}
          {!this.props.emailCapture && <EmailCapture siteVersion={this.props.siteVersion} name="Popup" buttonText="SUBMIT" onSignupSuccess={() => this.onSubmit()} />}

          <p>
            <FormattedMessage id="EmailCapturePopup.Seperator" defaultMessage="Or continue with" />
          </p>

          <IconButton icon={<FacebookIcon />} facebook white fullwidth rel="nofollow" url={`${global.__FAME_CONFIG__.URLS[this.props.siteVersion].api}/fb_auth?return_to=${encodeURI(this.getThankYouUrl(true))}`}>
            ...
            <div className="EmailCaptureModal__Wrapper" style={{ ...style, ...fadeStyle }}>
              ...
              <div className={'EmailCaptureModal'}>
                ...
                <div className="EmailCaptureModal__CloseWrapper">
                  <Button noBorder transparentBlack className="close" onClick={() => this.onClose()}>
