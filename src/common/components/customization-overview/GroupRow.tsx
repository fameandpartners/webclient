...
const HIDE_TITLE = [ComponentType.Color, ComponentType.ColorAndFabric, ComponentType.Fabric];
      ...
      <div className={classnames('GroupRow__Wrapper', { 'GroupRow__Wrapper--separators': includeSeparators, 'GroupRow__Wrapper--condensed': condensed })}>
        ...
        {!hideTitle && (
          <div className="title">
            ...
            {startCustomize && (
              <a className="start-customize-link" onClick={() => startCustomize(group as Group)}>
                {(group as Group).changeButtonText || <FormattedMessage id="change" defaultMessage="Change" />}
              </a>
            )}
          </div>
        )}
          ...
          {hasAggregateTitle ? (
            Object.entries(
              ...
              .map((sectionGroup, index) => {
                ...
                return <GroupRowSection key={index} title={sectionGroup.aggregateTitle} components={componentsInSectionGroup} customizedProduct={customizedProduct} condensed={condensed} siteVersion={siteVersion} />;
              })
          ) : (
            <GroupRowSection components={sectionGroups.map((sectionGroup) => (components as Component[]).filter(filterComponentInSectionGroup(sectionGroup as SectionGroup))).flatMap((x) => x)} customizedProduct={customizedProduct} condensed={condensed} siteVersion={siteVersion} />
          )}
        </div>
      </div>
    );
  ...
  public render() {
    ...
    return (
      <React.Fragment>
        ...
        <Desktop>{this.renderRow()}</Desktop>
      </React.Fragment>
    );
  }
}
