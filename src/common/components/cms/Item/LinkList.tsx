...
const ItemLinkList: React.SFC<Props> = ({ links }: Props) => {
  return (
    <BaseItem className="ItemLinkList" size={'Center'} mobileSize={'Center'} textAlignment={'Left'} textSize={'Medium'}>
      <style jsx>{`
        @import 'vars';
      ...
      <ul>
        {links.map((x, i) => (
          <li key={i}>
            <a className="no-underline" href={x.url} target={x.urlTargetBlank === true ? '_blank' : '_self'} rel="noopener noreferrer">
              {x.title}
            </a>
          </li>
        ))}
