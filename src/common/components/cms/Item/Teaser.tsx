...
const MaybeLink: React.SFC<{link: string| null, children: React.ReactNode}> = ({link, children}) => {
    if (link) {
        return <a href={link} className="no-underline">{children}</a>;
    }

    return <React.Fragment>{children}</React.Fragment>;
};

const Teaser: React.SFC<Props> = ({ title, media, video, size, mobileSize, link, textSize, textAlignment }: Props) => {
    return <BaseItem className="CmsTeaser" size={size} textAlignment={textAlignment} textSize={textSize} mobileSize={mobileSize}>
        <style jsx>{`
            @import 'vars';
